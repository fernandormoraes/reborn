...
Future<HttpBody> _process(Stream<List<int>> stream, HttpHeaders headers,
  ...
  Future<HttpBody> asBinary() async {
    var builder = await stream.fold<typed_data.BytesBuilder>(
        typed_data.BytesBuilder(), (builder, data) => builder..add(data));
  ...
  Future<HttpBody> asFormData() async {
    var values = await MimeMultipartTransformer(
      ...
      if (multipart.isText) {
        ...
        var buffer = await multipart.fold<typed_data.BytesBuilder>(
            typed_data.BytesBuilder(),
            (b, dynamic d) => b..add(d as List<int>));
